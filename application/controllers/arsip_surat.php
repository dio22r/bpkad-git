...
class arsip_surat extends CI_Controller {
    ...
    protected $activeMenu = "arsip_surat";
    ...
    public function index($search = "all", $start = 0) {
        ...
        $this->_cron_update_thumb(); // generate thumbnail
        ...
        $arrData = $this->tbl_arsip_surat->retrieve_by_limit($tempSearch, $start, $perpage);
        
        foreach  ($arrData as $key => $arrVal) {
            $arrData[$key]["url_view"] = $this->thisurl . "/view_data/" . $arrVal["as_id"];
            $arrData[$key]["url_edit"] = $this->thisurl . "/form/" . $arrVal["as_id"];

            $img = str_replace("sa-scan-file", "sa-scan-file/thumb", $arrVal["as_file"]);
            $arrData[$key]["as_file"] = base_url($img);
            $arrData[$key]["as_tgl_surat"] = misc_helper::format_idDate($arrVal["as_tgl_surat"]);
            $arrData[$key]["as_tgl_diterima"] = misc_helper::format_idDate($arrVal["as_tgl_diterima"]);
        }

        $this->load->library('pagination');

        $config['base_url'] = $this->thisurl."/index/".$search;
        $config['total_rows'] = $this->tbl_arsip_surat->count_search($tempSearch);
        ...
        $arrData = array(
            ...
            "ctlContentArea" => $this->load->view("arsip/vw_tbl_arsip", $contentData, true),
